...
const App: React.FC = () => {
  ...
  const allReports = useMemo(() => {
    return clients.flatMap(client =>
      (client.reports || []).map(r => ({ ...r, shopName: client.shopName }))
  ...
  const allTransactions = useMemo(() => {
    return clients.flatMap(client =>
      (client.history || []).map(job => ({ ...job, shopName: client.shopName, clientId: client.id }))
  ...
  const totalRevenue = useMemo(() => {
    return allTransactions.reduce((acc, tx) => {
      const amount = parseFloat(tx.cost.replace('₹', ''));
  ...
  const handleOnboard = (e: React.FormEvent) => {
    ...
    const newClient: Client = {
      ...
      email: isInstitution ? emailId : undefined
    };
  ...
  const handleResolveReport = (clientId: string, reportId: string) => {
    setClients(prev => prev.map(c => {
      if (c.id === clientId) {
        return {
          ...c,
          reports: (c.reports || []).map(r => r.id === reportId ? { ...r, status: 'resolved' } : r)
        };
      }
      return c;
    }));
  ...
  const handleAddPrinter = (e: React.FormEvent) => {
    ...
    if (printerToEdit) {
      ...
      const updatedClient = {
        ...selectedClient,
        printers: selectedClient.printers?.map(p => p.id === printerToEdit.id ? updatedPrinter : p) || []
      };
      ...
      const updatedClient = {
        ...selectedClient,
        printers: [...(selectedClient.printers || []), newPrinter]
      };
  ...
  const confirmDeletePrinter = () => {
    ...
    const updatedClient = {
      ...selectedClient,
      printers: selectedClient.printers?.filter(p => p.id !== printerToDelete) || []
    };
  ...
  const handleUpdateTransactionStatus = (clientId: string, jobId: string, newStatus: 'Paid' | 'Refunded') => {
    setClients(prev => prev.map(c => {
      if (c.id === clientId) {
        return {
          ...c,
          history: c.history?.map(job =>
      ...
      <div className="flex flex-1 min-w-0">
        <main className={`flex-1 min-w-0 p-4 lg:p-10 transition-all ${showDetailsPanel && selectedClient && activeView === 'customers' ? 'lg:mr-96' : ''}`}>
          ...
          <header className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-6">
            ...
            {/* Add Printer Button for Selected Client - Header */}
            {activeView === 'customers' && selectedClient && (
              <button onClick={() => setShowAddPrinterModal(true)} className="flex items-center justify-center gap-2 bg-black text-white px-6 py-3 rounded-full font-bold hover:bg-slate-800 transition-all shadow-xl shadow-black/10 active:scale-95 w-full md:w-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
            ...
            <StatCard label={selectedClient ? "Deployed Units" : "Deployed Units (Printers)"} value={selectedClient ? (selectedClient.printers?.length || 0).toString() : totalPrinters.toString()} icon={<LayoutGrid size={16} />} iconBg="bg-emerald-50" iconColor="text-emerald-600" />
            <StatCard label={selectedClient ? "Tickets" : "Open Reports"} value={selectedClient ? (selectedClient.reports?.filter(r => r.status === 'pending').length || 0).toString() : pendingReportsCount.toString()} icon={<MessageSquare size={16} />} iconBg="bg-amber-50" iconColor="text-amber-600" highlight={selectedClient ? (selectedClient.reports?.some(r => r.status === 'pending')) : pendingReportsCount > 0} />
          </div>
          ...
          {activeView === 'customers' && selectedClient && (
            <div className="space-y-10 animate-in fade-in duration-500">
              {/* Printers Section */}
              {selectedClient.printers && selectedClient.printers.length > 0 && (
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
                    {selectedClient.printers.map(printer => (
            ...
            <div className="space-y-6 animate-in fade-in duration-500">
              {allReports.map(report => (
                <div key={report.id} className="bg-white border rounded-3xl p-8 hover:shadow-lg transition-all flex flex-col lg:flex-row gap-6 items-center">
                  ...
                  {report.status === 'pending' && <button onClick={() => { const c = clients.find(cl => cl.shopName === report.shopName); if (c) handleResolveReport(c.id, report.id) }} className="bg-black text-white px-8 py-3 rounded-xl font-bold">Resolve Ticket</button>}
                </div>
              ))}
            </div>
          )}
                  ...
                  <div>
                    <div className="grid grid-cols-2 gap-3">
                      <div className="bg-blue-50 rounded-xl p-3">
                        ...
                        <p className="text-xl font-bold text-blue-900">{selectedClient.printers?.length || 0}</p>
                      </div>
                      <div className="bg-indigo-50 rounded-xl p-3">
                        ...
                        <p className="text-xl font-bold text-indigo-900">{selectedClient.history?.reduce((acc, curr) => acc + curr.pages, 0) || 0}</p>
                      </div>
                      <div className="bg-amber-50 rounded-xl p-3">
                        ...
                        <p className="text-xl font-bold text-amber-900">{selectedClient.reports?.filter(r => r.status === 'pending').length || 0}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  {selectedClient.history && selectedClient.history.length > 0 && (
                    <div>
                      ...
                      <div className="space-y-2">
                        {selectedClient.history.slice(0, 3).map((job) => (
